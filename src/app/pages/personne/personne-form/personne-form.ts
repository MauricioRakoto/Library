          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-sm text-gray-500 font-medium">
                {{ isEdit ? 'Modification : ' + personne.nom_per + ' ' + personne.prenom_per : 'Informations personnelles de l\'adhérent' }}
              </p>
            </div>
          </div>
        </div>
          ...
          <form (ngSubmit)="submit()" class="p-10 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="space-y-2">
                ...
                <input type="text" [(ngModel)]="personne.nom_per" name="nom_per"
              ...
              <div class="space-y-2">
                ...
                <input type="text" [(ngModel)]="personne.prenom_per" name="prenom_per"
                       placeholder="ex: Jean"
                       class="w-full px-5 py-3.5 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-4 focus:ring-green-500/10 focus:border-green-500 focus:bg-white transition-all font-semibold" />
              </div>
            </div>
            ...
            <div class="space-y-2">
              ...
              <div class="relative">
                ...
                <input type="text" [(ngModel)]="personne.adresse" name="adresse"
...
export class PersonneForm implements OnInit {
  ...
  ngOnInit() {
    ...
    if (id) {
      ...
      this.service.getById(+id).subscribe({
        next: (data: Personne) => {
          this.personne = data;       // ← charger les données dans le formulaire
          this.loading  = false;
        ...
        error: (err) => {
          ...
          this.loading = false;
          this.erreur  = 'Impossible de charger les données de cette personne.';
  ...
  submit() {
    if (!this.personne.nom_per || !this.personne.prenom_per) {
    ...
    if (this.isEdit) {
      this.service.update(this.personne.personne_id!, this.personne).subscribe({
        next: () => this.router.navigate(['/personnes']),
        error: (err) => {
      ...
      this.service.create(this.personne).subscribe({
        next: () => this.router.navigate(['/personnes']),
        error: (err) => {
