        ...
        <div class="px-10 py-8 bg-slate-50/50 border-b border-gray-100 flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-500 font-medium">
              {{ isEdit ? 'Modification : ' + livre.nom_livre : 'Référencement du livre dans la base de données' }}
            </p>
          </div>
            ...
            <div class="space-y-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                  ...
                  <input type="text" [(ngModel)]="livre.nom_livre" name="nom_livre"
                ...
                <div class="space-y-2">
                  ...
                  <input type="text" [(ngModel)]="livre.auteur" name="auteur"
              ...
              <div class="space-y-2">
                ...
                <textarea [(ngModel)]="livre.desc" name="desc"
                          placeholder="Décrivez brièvement le contenu du livre..." rows="3"
...
export class LivreForm implements OnInit {
    ...
    if (id) {
      ...
      this.service.getById(+id).subscribe({
        next: (data: Livre) => {
          this.livre   = data;
          this.loading = false;
          this.cdr.detectChanges();  // ← afficher les données dans le formulaire
        },
        error: (err) => {
          console.error('Erreur chargement livre:', err);
          this.loading = false;
          this.erreur  = 'Impossible de charger le livre.';
          this.cdr.detectChanges();
        }
      });
    }
  ...
  submit() {
    if (!this.livre.nom_livre || !this.livre.auteur) {
      this.erreur = 'Le titre et l\'auteur sont obligatoires.';
      return;
    }
    this.erreur = '';
    ...
    const payload: Livre = {
      ...this.livre,
      matiere_id: +this.livre.matiere_id,  // ← forcer en number
    };

    if (this.isEdit) {
      this.service.update(payload.livre_id!, payload).subscribe({
